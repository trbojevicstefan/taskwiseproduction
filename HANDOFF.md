...
- `npm test -- --runInBand` — 115 suites / 568 tests, all passing (baseline was 86/272)
...
- New indexes were added to `scripts/create-performance-indexes.js` (tasks `dueAt`, meetings `startTime`, and four `taskReminders` indexes incl. the unique dedup index) — **run `npm run db:indexes:perf` against production Mongo when deploying**. The lib also self-ensures `taskReminders` indexes at runtime via `ensureTaskReminderIndexes`.
...
- Remaining phases: 7 must keep all fathom-* tests green while extracting the provider abstraction; 8 should generate `/docs/mcp` from a tool registry to stop doc drift.
