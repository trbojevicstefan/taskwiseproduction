...
| 5 | Planning workspace | ✅ done | see `git log` (`feat: Phase 5`) |
| 10 | Slack scheduled reminders | ⬜ pending | — |
| 7 | Fireflies + Grain providers | ⬜ pending | — |
...
- `npm run lint` — 0 errors, 51 warnings (baseline was 66; dead-code deletion reduced it)
- `npm test -- --runInBand` — 110 suites / 523 tests, all passing (baseline was 86/272)
...
- Phase 5 (in progress) reuses `POST /api/ai/chat` for the planning assistant instead of a new flow, and moves the existing Google Meeting Planner to `/planning/agendas`.
- `package-lock.json` churn from a local `npm install` was deliberately reverted; the committed lockfile is untouched.
- New indexes were added to `scripts/create-performance-indexes.js` (tasks `dueAt`, meetings `startTime`) — **run `npm run db:indexes:perf` against production Mongo when deploying**.
...
- Remaining phases: 10 needs a decision on the reminder trigger story (job worker vs cron route — see plan doc); 7 must keep all fathom-* tests green while extracting the provider abstraction; 8 should generate `/docs/mcp` from a tool registry to stop doc drift.
