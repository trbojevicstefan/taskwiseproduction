...
const TASK_COMPLETION_LLM_SNIPPET_CAP = Math.min(
  12,
  Math.max(4, Number(process.env.TASK_COMPLETION_LLM_SNIPPET_CAP || 4))
);
...
const upsertCandidate = (
  ...
  target: CompletionCandidate["targets"][number],
  allowUnassigned: boolean
) => {
  if (
    !payload.title ||
    !isValidTitle(payload.title) ||
    isPlaceholderTitle(payload.title)
  ) {
    return;
  }
  ...
  const shouldReplace =
    ...
    (payload.description &&
      payload.description.length > existing.description.length) ||
...
export const buildCompletionSuggestions = async ({
  ...
  const tasks = await db.collection("tasks").find(taskFilters).toArray();
  ...
  if (!completionSnippets.length) {
    debugInfo.selection.selectionThreshold = Math.min(
      0.95,
      Math.max(0.4, minMatchRatio)
    );
  ...
  for (const review of snippetsForModel) {
    ...
    if (!completedItems.length) {
      if (
        ...
        review.topScore >= minimumCandidateScore + 0.05
      ) {
        mergeCompletion({
  ...
  const resolveFallbackCandidates = async () => {
    ...
    for (const entry of unmatched) {
      ...
      results.push({
        ...
        priority: (bestCandidate.priority as ExtractedTaskSchema["priority"]) ||
          "medium",
