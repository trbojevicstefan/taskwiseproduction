...
import type { StyleOption } from '@/types/paste-options';
...
const sanitizeTaskForInternalState = (task: any): DisplayTask => {
  ...
  return {
    ...
    assignee: task.assignee === undefined ? null : task.assignee,
...
const styleOptions: { id: StyleOption, label: string, icon: React.ElementType }[] = [
...
export default function PlanningPageContent() {
  ...
  const { showCopyHint, setShowCopyHint } = useUIState();
 ...
 const handleBreakDownTask = async (taskToBreakDown: DisplayTask) => {
  ...
  const handleGenerateBriefsForSelectedTasks = async () => {
    ...
    const briefPromises = Array.from(selectedTaskIds).map(async (taskId) => {
        ...
        if (taskToUpdate) {
            try {
                const briefInput: GenerateResearchBriefInput = {
                    taskTitle: taskToUpdate.title,
                    taskDescription: taskToUpdate.description || undefined,
                };
                const briefResult = await generateResearchBrief(briefInput);
          ...
          {isInputAreaVisible && (
            <div className="space-y-3">
                <Alert className="border-primary/30 bg-primary/5">
                  ...
                  <AlertTitle>Start Your Plan</AlertTitle>
                  <AlertDescription>
                    Paste your meeting notes, project brief, or any text to have AI structure it into a plan. You can also paste anywhere in the app with <kbd className="px-1.5 py-0.5 border rounded bg-muted font-mono text-xs">Ctrl+V</kbd>.
                  </AlertDescription>
                </Alert>
