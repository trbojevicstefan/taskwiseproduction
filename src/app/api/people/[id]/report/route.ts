...
const ROUTE = "/api/people/[id]/report";
...
/**
 * One-click person report: gathers the person's meetings, tasks, and
 * transcript mentions, then composes a source-grounded report via gpt-4o-mini
 * (runPromptWithFallback). No evidence => deterministic report without any
 * LLM call. LLM-cited sources are filtered against the gathered ids,
 * mirroring the /api/ai/chat anti-hallucination contract.
 */
export async function POST(
  ...
  try {
    ...
    if (!person) {
      emitMetric(404, "error", { reason: "person_not_found" });
      return apiError(404, "request_error", "Person not found", undefined, {
        correlationId,
      });
    }
    ...
    const evidence = await gatherProfileReportEvidence(
      ...
      { type: "person", name: personName, people: [person] }
    );

    if (evidence.isEmpty) {
      const data = buildNoEvidenceReport("person", personName);
    ...
    return mapApiError(error, "Failed to generate person report.", {
      correlationId,
      logger,
