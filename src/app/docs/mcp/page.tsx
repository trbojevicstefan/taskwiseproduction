...
export const metadata: Metadata = {
  ...
  description:
    "TaskWiseAI MCP access docs: authentication, endpoints, JSON-RPC methods, tools, scopes, and rate limits.",
};
...
const RPC_METHODS: RpcMethod[] = [
  ...
  },
  {
    ...
    scope: "mcp:read",
    notes: "Currently returns an empty list.",
  },
];
...
const READ_TOOLS: ToolDefinition[] = [
  {
    ...
    scope: "mcp:read",
    args: "{}",
    notes: "Returns most recent non-hidden meeting for the workspace.",
  },
  {
    name: "meetings.list",
    scope: "mcp:read",
    args: "{ limit?: number (1-100) }",
    notes: "Returns recent non-hidden meetings (default limit: 20).",
  },
  {
    name: "meetings.get",
    scope: "mcp:read",
    args: "{ meetingId: string }",
    notes: "Loads one meeting by ID.",
  },
  {
    name: "action_items.list",
    scope: "mcp:read",
    args:
      "{ limit?, status?, includeDone?, includeSubtasks?, personId?, sourceSessionType? }",
    notes: "Lists workspace action items with optional filters.",
  },
  {
    name: "people.list",
    scope: "mcp:read",
    args: "{ limit?: number (1-100), query?: string }",
    notes: "Lists people records for the workspace.",
  },
  {
    name: "people.get",
    scope: "mcp:read",
    args: "{ personId: string, includeActionItems?: boolean, actionItemsLimit?: number }",
    notes: "Returns person details and optionally assigned action items.",
  },
];
...
const WRITE_TOOLS: ToolDefinition[] = [
  {
    ...
    scope: "mcp:write",
    args: "{ taskId: string, status: 'todo' | 'inprogress' | 'done' | 'recurring' }",
    notes: "Updates action item status.",
  },
  {
    name: "action_items.update_assignee",
    scope: "mcp:write",
    args: "{ taskId: string, assignee?: { uid?, email?, name? } | null, assigneeName?: string | null }",
    notes: "Updates/clears assignee fields.",
  },
  {
    name: "action_items.update_due_date",
    scope: "mcp:write",
    args: "{ taskId: string, dueAt: string | null }",
    notes: "Updates due date (ISO date string) or clears it.",
  },
  {
    name: "action_items.update_notes",
    scope: "mcp:write",
    args: "{ taskId: string, notes: string | null }",
    notes: "Updates notes/comments.",
  },
  {
    name: "action_items.update_title",
    scope: "mcp:write",
    args: "{ taskId: string, title: string }",
    notes: "Updates canonical action item title.",
  },
];
      ...
      <main className="mx-auto max-w-5xl space-y-8 px-4 py-8 sm:px-6 sm:py-10">
        <section className="space-y-3">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground sm:text-base">
            ...
            authentication, all MCP endpoints, JSON-RPC methods, available tools, rate limits, and
            error handling.
          </p>
        </section>
        ...
        <section className="space-y-3 rounded-lg border bg-muted/20 p-4">
          ...
          <ol className="list-decimal space-y-1 pl-5 text-sm text-muted-foreground">
            ...
            <li>
              Grant <code>mcp:write</code> only if the client must edit action items.
            </li>
          </ol>
        </section>
...
X-API-Key: <YOUR_MCP_KEY>`}
        ...
        <section className="space-y-3">
          <h2 className="text-xl font-semibold">Read Tools ({`mcp:read`})</h2>
          <Table
            headers={["Tool", "Scope", "Arguments", "Notes"]}
            rows={READ_TOOLS.map((tool) => [tool.name, tool.scope, tool.args, tool.notes])}
          />
          <p className="text-xs text-muted-foreground">
            Compatibility aliases are accepted for older clients: <code>attendees.list</code> and{" "}
            <code>attendees.get</code>.
          </p>
        </section>

        <section className="space-y-3">
          <h2 className="text-xl font-semibold">Write Tools ({`mcp:write`})</h2>
          <Table
            headers={["Tool", "Scope", "Arguments", "Notes"]}
            rows={WRITE_TOOLS.map((tool) => [tool.name, tool.scope, tool.args, tool.notes])}
          />
          <p className="text-xs text-muted-foreground">
...
{`curl -X POST "https://www.taskwise.ai/api/workspaces/<workspaceId>/mcp" \\
  ...
  -d '{
    ...
    "params": {}
          ...
          <p className="text-sm text-muted-foreground">List tools:</p>
          <pre className="overflow-x-auto rounded-lg border bg-muted/20 p-3 text-xs">
{`curl -X POST "https://www.taskwise.ai/api/workspaces/<workspaceId>/mcp" \\
  -H "Content-Type: application/json" \\
  -H "Authorization: Bearer <YOUR_MCP_KEY>" \\
